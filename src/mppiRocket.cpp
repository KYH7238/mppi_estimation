...
void MPPI::publishTrajectory(const Eigen::MatrixXd &trajectory) {
    ...
    for (int j = 0; j < trajectory.cols(); j++) {
        ...
        pose.pose.position.x = trajectory(0, j);
        pose.pose.position.y = trajectory(1, j);
...
double MPPI::p(const Eigen::VectorXd& x, const Eigen::VectorXd& x_target) {
    ...
    double theta_diff = x(4) - x_target(4); 
    double w_diff = x(5) - x_target(5);
    return 1e-2 * (p_diff.squaredNorm() + v_diff.squaredNorm() + theta_diff * theta_diff + w_diff * w_diff);
}
