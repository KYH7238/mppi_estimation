...
Eigen::MatrixXd MPPI::getNoise(const int &T) {
    return sigma_u * Eigen::MatrixXd::Random(dim_u, T);
}
...
void MPPI::solve() {
    ...
    Eigen::VectorXd costs = Eigen::VectorXd::Zero(N);
    Eigen::VectorXd weights = Eigen::VectorXd::Zero(N);
    std::vector<Eigen::MatrixXd> allTrajectories(N, Eigen::MatrixXd(dim_x, T+1));
    ...
    publishTrajectory(Xo);
    saveCost(min_cost);

    auto finish = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = finish - start;
    ROS_INFO("solve time: %f sec", elapsed.count());
...
void MPPI::publishTrajectory(const Eigen::MatrixXd &trajectory) {
    ...
    for (int j = 0; j < trajectory.cols(); j++) {
        ...
        pose.pose.position.x = trajectory(1, j);
        pose.pose.position.y = trajectory(0, j);
...
double MPPI::q(const Eigen::VectorXd& x, const Eigen::VectorXd& u) {
    double control_cost = 2e-5 * u.squaredNorm();
    double state_cost = 5e-3 * (x(1) * x(1));
    // double state_cost = 5e-3 * x.head(2).norm();
    double alpha = std::atan2(u(1), u(0)) * 180 / M_PI;  
    double alpha_cost = C * alpha * alpha;
    return control_cost + state_cost + alpha_cost;
}
...
void MPPI::h(Eigen::Ref<Eigen::MatrixXd> U_seq) {
    ...
    for (int j = 0; j < U_seq.cols(); ++j) {
        ...
        double proj_u0 = a;
        double proj_u1 = t * v;
        double norm = std::sqrt(proj_u0*proj_u0 + proj_u1*proj_u1);

        if (norm < F_min) {
            double scale = F_min / norm;
            proj_u0 *= scale;
            proj_u1 *= scale;
        } 
        else if (norm > F_max) {
            double scale = F_max / norm;
            proj_u0 *= scale;
            proj_u1 *= scale;
        }
